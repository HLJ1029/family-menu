...
const checks = [
  ...
  },
  {
    ...
    ok: packageJson.includes("release:candidate:dispatch:workbench")
      ...
      && candidateDispatchWorkbench.includes("CURRENT_UPLOADED_EXPERIENCE_VERSION")
      && candidateDispatchWorkbench.includes("EXPERIENCE_ENTRY")
      && candidateDispatchWorkbench.includes("历史二维码已禁用")
      && candidateDispatchWorkbench.includes("directPreviewOk: false")
      ...
      && candidateDispatchWorkbenchSelftest.includes("must not auto-discover historical share evidence")
      && candidateDispatchWorkbenchSelftest.includes("must fail closed for historical share QR files")
      && candidateDispatchWorkbenchSelftest.includes("must not expose historical share QR images")
      && candidateDispatchWorkbenchSelftest.includes("experience-version entry")
      && candidateDispatchWorkbenchSelftest.includes("must not render any historical QR as ready")
      ...
      && candidateDispatchWorkbenchSelftest.includes("must not suggest stale direct-preview generation")
