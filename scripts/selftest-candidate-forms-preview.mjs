...
assert(html.includes('data-form="send-checklist"'), "preview missing send checklist marker");
assert(html.includes("体验者只看反馈单"), "preview missing tester-only confirmation");
assert(html.includes("执行人保留记录单"), "preview missing host-run-sheet confirmation");
assert(html.includes("隐私留在仓库外"), "preview missing privacy confirmation");
assert(html.includes("未达标不审核"), "preview missing no-review-before-validation confirmation");
