...
const REQUIRED_SCREENSHOTS = [
  ...
  ["wish-landing.png", "wish token 打开后的免登录写菜落地页截图"],
  ["menu-card.png", "menu 小程序分享卡片预览截图"],
  ["menu-landing.png", "menu token 打开后的免登录菜单落地页截图"],
];
...
const TRIGGER_GUIDE = [
  ...
  },
  {
    ...
    steps: [
      ...
      "点击清单页的买菜分享动作。",
      "点击页面里的分享动作，或点小程序右上角菜单里的转发。",
      "分享预览标题应为“某某发来买菜清单/若干项买菜清单”的语义，path 应带 `?groceryShare=` token。",
    ],
  },
  {
    key: "wish",
    ...
    steps: [
      ...
      "点击分享入口并进入原生分享确认页。",
      "点击黑色分享按钮。",
      "分享预览标题应为“某某想收集家里最近想吃的菜”的语义，path 应带 `?wishShare=` token。",
    ],
  },
  {
    key: "menu",
    ...
    steps: [
      ...
      "点击“分享菜单”并进入原生分享确认页。",
      "点击黑色分享按钮。",
      "分享预览标题应展示今晚菜名，path 应带 `?menuShare=` token。",
    ],
  },
];
...
const lines = [
  ...
  "3. 如 H5 内分享入口不稳定，先运行 npm run release:wechat:share:direct-previews 生成五张直达原生分享确认页的预览二维码。",
...
function buildChecklist({ evidenceDir, previewQr, shareExpectations, screenshotRows }) {
  ...
  return [
    ...
    "- 落地页不被登录墙挡住，crave/grocery/wish 可免登录参与，menu 可免登录查看，invite 能进入加入家庭流程。",
    ...
    "- 若 H5 内点击分享没有跳到原生确认页，可运行 `npm run release:wechat:share:direct-previews` 生成 `direct-preview/*-preview-qr.png`，分别直达 `pages/share/index` 的五类确认页。",
    ...
    "- 人工视觉确认五张原生 card 后，运行 `npm run release:wechat:share:complete` 勾选提审前 P1。",
