...
const REQUIRED_CHECKS = [
  ...
  },
  {
    ...
    title: "主厨可在家庭设置中创建并切换独立家庭数据",
    path: "src/components/HouseholdSettingsPage.jsx",
    required: ["household-settings-page", "household-switcher", "onSwitchHousehold", "onCreateHousehold", "例如：爸妈家", "新建一个家", "canManageHousehold", "role=\"alert\"", "createPending", "trim()", "请填写家庭名称。"],
    evidence: "scripts/smoke-product-entrypoints.mjs",
    evidenceRequired: ["multi-household-owner-creates-via-family-settings", "multi-household-new-family-starts-empty", "multi-household-switches-from-household-settings", "member-cannot-create-another-household", "multi-household-page-errors"],
  },
  {
    key: "multi-household-user-center-wiring",
    title: "我的家只把新建家庭动作转发给家庭设置",
    path: "src/components/UserCenter.jsx",
    required: ["onCreateHousehold", "<HouseholdSettingsPage", "onCreateHousehold={onCreateHousehold}"],
    evidence: "scripts/smoke-product-entrypoints.mjs",
    evidenceRequired: ["multi-household-owner-creates-via-family-settings"],
  },
  {
    key: "multi-household-submit-integrity",
    title: "新建第二个家拒绝空名且只消费服务端 envelope",
    path: "src/main.jsx",
    required: ["createAnotherHumiHousehold", "String(name || \"\").trim()", "请填写家庭名称。", "return { ok: false", "return { ok: true", "applyHumiStateEnvelope(data,"],
    forbidden: ["String(name || \"另一个家\").trim() || \"另一个家\""],
    evidence: "scripts/smoke-product-entrypoints.mjs",
    evidenceRequired: ["blankCreateRequests", "failedInputPreserved", "createRequestBody", "newFamilyStateIsolated", "originalStateRestored"],
  },
  {
  ...
  },
  {
    ...
    title: "想吃征集完成只承诺绑定参与身份",
    path: "src/components/CraveLanding.jsx",
    required: ["onBindParticipation", "登录 Humi，保存这次参与", "不会自动成为家庭成员", "另行接受家庭邀请"],
    forbidden: ["onJoinFamily", "加入这个家"],
    evidence: "scripts/smoke-collaboration-landings.mjs",
    evidenceRequired: ["crave-completion-promises-identity-binding-only", "crave-cta-stores-typed-pending-participation"],
  },
  {
    key: "grocery-participation-identity-binding",
    title: "买菜认领完成只承诺绑定参与身份",
    path: "src/components/GroceryClaimLanding.jsx",
    required: ["onBindParticipation", "登录 Humi，保存这次参与", "不会自动成为家庭成员", "另行接受家庭邀请"],
    forbidden: ["onJoinFamily", "加入这个家"],
    evidence: "scripts/smoke-collaboration-landings.mjs",
    evidenceRequired: ["grocery-completion-promises-identity-binding-only", "grocery-cta-stores-typed-pending-participation"],
  },
  {
    key: "wish-participation-identity-binding",
    title: "最近想吃完成只承诺绑定参与身份",
    path: "src/components/WishLanding.jsx",
    required: ["onBindParticipation", "登录 Humi，保存这次参与", "不会自动成为家庭成员", "另行接受家庭邀请"],
    forbidden: ["onJoinFamily", "加入这个家"],
    evidence: "scripts/smoke-collaboration-landings.mjs",
    evidenceRequired: ["wish-completion-promises-identity-binding-only", "wish-cta-stores-typed-pending-participation"],
  },
  {
    key: "generic-participation-handler-semantics",
    title: "通用协作回调只绑定参与，不冒充家庭邀请",
    path: "src/main.jsx",
    required: ["bindParticipationFromSharedLanding", "onBindParticipation"],
    forbidden: ["joinFamilyFromSharedLanding", "onJoinFamily="],
    evidence: "scripts/smoke-collaboration-flow.mjs",
    evidenceRequired: ["登录 Humi，保存这次参与", "binding participation must not create a formal household member"],
  },
  {
    key: "real-household-invite-keeps-membership-action",
    title: "真实家庭邀请保留明确加入动作",
    path: "src/components/InviteLanding.jsx",
    required: ["加入这个家", "joinHouseholdInvite", "加入后才会成为正式家庭成员"],
    evidence: "scripts/smoke-collaboration-landings.mjs",
    evidenceRequired: ["invite-shows-value-before-login"],
  },
  {
  ...
  },
  {
    ...
    evidenceRequired: [
      ...
      "multi-household-owner-creates-via-family-settings",
      "multi-household-new-family-starts-empty",
