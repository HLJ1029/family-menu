...
const result = {
  ...
  nextActions: [
    ...
    "After sending, use npm run release:candidate:invite -- --from-dispatch YYYY-MM-DD to mark anonymous U ids as 已邀请.",
...
function buildReadme({ git, candidate, status }) {
  return [
    ...
    "- `npm run release:candidate:invite -- --from-dispatch YYYY-MM-DD`：发送当天分发单后，把对应匿名 U 编号标为已邀请；不记录真实联系人。",
    ...
    "5. 发送后运行 `npm run release:candidate:invite -- --from-dispatch YYYY-MM-DD`，把当天 U 编号标为已邀请。",
