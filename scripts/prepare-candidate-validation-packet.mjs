...
function buildReadme({ git, candidate, status }) {
  return [
    ...
    "4. 发 `tester-feedback-form.md` 的问题给体验者；执行人自己用 `host-run-sheet.md` 记录现场观察。",
    "5. 每个用户至少跑一次【今晚】推荐、今晚菜单、清单，尽量再跑一次问问大家/邀请家人/买菜认领。",
    "6. 把单个用户结果汇总到 `feedback-template.csv`，并在每天结束时用 `npm run release:candidate:daily -- --date YYYY-MM-DD` 自动写 `daily-review.csv`。",
    ...
    "7. 任一 P0 或多个用户同一核心链路卡住，先暂停审核准备，进入修复。",
    "8. 每轮回填后运行 `npm run release:candidate:doctor` 看还差哪些样本。",
    "9. 内测无 P0/P1 且 `npm run release:candidate:review` 通过后，再由用户明确确认是否进入微信审核。",
    "",
    "## 候选复盘最低通过标准",
    "",
    "- 默认至少 10 个真实体验样本。",
