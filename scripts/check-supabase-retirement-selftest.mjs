...
await writeFile(join(unsafeRoot, "package.json"), '{"dependencies":{"@supabase/auth-js":"1.0.0"},"scripts":{"build":"node scripts/supabase-export.js"}}\n');
...
await writeFile(join(unsafeRoot, ".postcssrc"), '{"endpoint":"https://legacy.supabase.co"}\n');
const unsafeFindings = await checkSupabaseRetirement(unsafeRoot);
assert.ok(unsafeFindings.some((item) => item === "dependency:@supabase/auth-js"), "every provider dependency must fail");
...
assert.ok(unsafeFindings.some((item) => item === "config:.postcssrc"), "standard rc residue must fail");
