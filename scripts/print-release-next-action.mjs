...
if (openHardeningItems.length) {
  ...
  lines.push("5. 一天结束时运行 npm run release:candidate:daily -- --date YYYY-MM-DD，自动把当天复盘写入 daily-review.csv。");
  lines.push("6. 运行 npm run release:candidate:record:selftest 和 npm run release:candidate:daily:selftest，确认回填工具仍能写入临时私有执行包。");
  lines.push("7. 运行 npm run release:candidate:review，确认达到 10 个真实体验、8 个今晚菜单、8 个清单、3 个协作样本且无 P0/P1。");
  lines.push("8. 若出现 P0/P1，先修复或明确进入 1.1.x，再回到候选复盘；不要绕过内测直接审核。");
  lines.push("9. 候选复盘达标后，再由用户动作当下确认是否进入微信审核准备。");
} else if (wechat?.ok) {
