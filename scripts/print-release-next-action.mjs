...
if (openHardeningItems.length) {
  ...
  lines.push("3. 运行 npm run release:candidate:doctor，看当前 U001-U020 真实反馈、核心路径完成和协作样本还差多少。");
  lines.push("4. 收到单个体验者反馈后，用 npm run release:candidate:record -- --user U001 ... 回填匿名汇总，真实姓名、微信号、手机号和截图继续留在仓库外。");
  lines.push("5. 运行 npm run release:candidate:record:selftest，确认回填工具仍能写入临时私有执行包。");
  lines.push("6. 运行 npm run release:candidate:review，确认达到 10 个真实体验、8 个今晚菜单、8 个清单、3 个协作样本且无 P0/P1。");
  lines.push("7. 若出现 P0/P1，先修复或明确进入 1.1.x，再回到候选复盘；不要绕过内测直接审核。");
  lines.push("8. 候选复盘达标后，再由用户动作当下确认是否进入微信审核准备。");
} else if (wechat?.ok) {
...
lines.push("- 批量邀请清单：release:candidate:prepare 会生成 outreach-batch.md，可直接复制 U001-U010 的匿名邀请消息。");
  ...
  if (missing.includes("## 4. 微信公众平台提交审核证据")) {
    ...
    return {
      ...
      actions: [
        ...
        "收到单个体验者反馈后，用 npm run release:candidate:record -- --user U001 ... 回填匿名汇总；运行 npm run release:candidate:record:selftest 确认工具可用。",
