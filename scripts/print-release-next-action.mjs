...
if (openHardeningItems.length) {
  ...
  if (candidateAction.dispatch) {
    if (candidateAction.dispatch.allUsersInvited) {
      ...
      lines.push("2. 收到单个用户反馈后，先运行 `release:candidate:record:draft -- --user U00X --date YYYY-MM-DD --entry \"入口任务\"` 生成私有回填草稿，再替换真实匿名结果并回填；不要原样运行占位模板。");
...
function getCompletionCriteria(scope) {
  const candidateCriteria = [
    ...
    "今日分发工作台：npm run release:candidate:dispatch:workbench -- --date YYYY-MM-DD 会在私有包生成 candidate-dispatch-workbench-YYYY-MM-DD.html，把逐个发送文案、小程序卡片发送确认、每个 U 的已发送登记命令、回填草稿命令、回填模板和日结命令放进同一个可复制页面；它会显示待邀请/已邀请/已体验状态，并在部分 U 已邀请时只生成未邀请 U 编号的批量发送标记命令；它不会发送消息或自动标记邀请。",
    "反馈回填草稿：npm run release:candidate:record:draft -- --user U00X --date YYYY-MM-DD 会在私有包生成 candidate-record-draft-U00X-YYYY-MM-DD.md，先把必填字段、占位符和 record 命令整理出来；它不会写入反馈 CSV。",
...
function getNextEvidenceStage(missing, submitEvidence) {
  if (missing.includes("## 4. 微信公众平台提交审核证据")) {
    ...
    return {
      ...
      actions: [
        ...
        "收到单个体验者反馈后，先运行 npm run release:candidate:record:draft -- --user U001 --date YYYY-MM-DD 生成私有回填草稿，再替换真实匿名结果并用 npm run release:candidate:record -- --user U001 ... 回填匿名汇总；一天结束时运行 npm run release:candidate:day:close -- --date YYYY-MM-DD 写入 daily-review.csv 和私有收尾报告。",
        ...
        `运行 npm run release:wechat:check 只做只读预检，确认版本 ${CURRENT_MINIPROGRAM_VERSION}、域名、隐私保护指引、审核备注和证据目录仍可用。`,
  ...
  if (missing.includes("## 5. 审核结果证据")) {
    return {
      ...
      actions: [
        `等待微信公众平台给出 ${CURRENT_MINIPROGRAM_VERSION} 审核结果。`,
  ...
  if (missing.includes("## 6. 审核通过后发布证据")) {
    return {
      ...
      actions: [
        `如果审核结果是通过，进入微信公众平台版本管理，找到 ${CURRENT_MINIPROGRAM_VERSION} 并点击发布。`,
  ...
  if (missing.includes("## 7. 发布后 P0 真机验收证据")) {
    return {
      title: `${CURRENT_MINIPROGRAM_VERSION} 已发布，下一步是真机 P0 验收。`,
      actions: [
