...
export function GroceryClaimLanding({ token, humiSession, onClose, onBindParticipation }) {
  ...
  return (
    <LandingShell>
      <main data-testid="grocery-share-landing" className="mx-auto grid min-h-screen w-full max-w-2xl content-center px-5 py-8">
        <section className="rounded-[32px] border border-line bg-white p-5 shadow-lift sm:p-8">
          ...
          {claimed ? (
            <div className="mt-6 rounded-[24px] bg-canvas p-5">
              ...
              <div className="mt-5 grid gap-2 sm:grid-cols-2">
                {participant?.type === "guest" && <button type="button" onClick={() => onBindParticipation?.({ type: "grocery", token, guestParticipantId: participant.id, actionId: participant.actionId || "", householdName: request.householdName || "我家", initiatorName: request.initiatorName || "主厨", claimStatus, itemCount: claimStatus === "declined" ? 0 : selectedItemIds.length || items.length })} className="min-h-12 rounded-full bg-ink px-6 py-3 text-sm font-black text-white">登录 Humi，保存这次参与</button>}
                <button type="button" onClick={onClose} className="min-h-12 rounded-full border border-ink bg-white px-6 py-3 text-sm font-black text-ink">
