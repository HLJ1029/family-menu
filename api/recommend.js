...
const SYSTEM_PROMPT =
  ...
  "必须只从候选菜谱中选择，不能创造新菜。优先考虑：1. 忌口和过敏绝对禁止；2. 快到期和家里现有的主食材；3. 避开近期重复菜；4. 总耗时尽量不超过45分钟；" +
...
export async function generateMealRecommendation(payload, options = {}) {
  ...
  return {
    recipeIds,
    reason: String(parsed.reason ?? payload.ruleFallback?.reason ?? "已按家里现有和偏好生成推荐。"),
    explanation: {
      pantry: String(parsed.pantry ?? "已参考家里现有。"),
