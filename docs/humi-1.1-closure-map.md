...
Humi 1.1 已形成可供真机验收的生产候选：核心家庭菜单与协作改动按当前重构 UI 完成，小程序候选为 `1.1.68`。微信审核和正式发布尚未触发；下一步只验证真实微信中的打开、分享和家庭协作，不再扩大 1.1 功能范围。
...
| 部署 H5/API 并上传新小程序候选 | 10 | 10 | `1.1.68` 以当前重构 UI 和完整功能门禁为基线；部署、上传与线上 smoke 证据记录在发布证据日志 |
| 用户手机 P0 打开与核心路径 | 5 | 0 | `1.1.61` 真机白屏已修复；`1.1.68` 需用户手机重新扫码确认，不用开发者工具结果代替 |
| 真实家庭灰度达到最低样本线 | 10 | 0 | 当前 0/10 真实体验、0/8 完成今晚、0/8 完成清单、0/3 尝试协作 |
| 微信审核、发布与发布后 24 小时监控 | 10 | 0 | 审核、发布、发布后真机和 24 小时监控均未执行，等待候选验收后用户确认 |
| **完整上线总计** | **100** | **75** | **工程与部署候选已完成；完整上线完成度 75%；小程序 `1.1.68` 候选仍待用户手机 P0、真实家庭灰度和审核发布** |
...
- 小程序候选：`1.1.68` / `修复分享并优化应用文案` / AppID `wx4040b89f3b363416`。
...
先完成产品功能，再谈审核。
先用微信开发者工具或体验版打开 `1.1.68`，完成 1.1 核心菜单与协作真机验收。
