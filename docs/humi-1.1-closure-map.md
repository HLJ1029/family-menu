...
Humi 1.1 功能、体验、提审前 P0/P1、生产 API、H5 部署、小程序上传和三类小程序原生分享卡片证据都已完成；当前阶段是继续做 1.1 生产候选完善与内测验证，不自动提交微信审核，最后再由用户确认是否进入微信公众平台审核。
...
- 当前工程门禁：`npm run release:status` 为 `ok=true`；`npm run release:wechat:check` 为 `ok=true`；`release:status` 已覆盖文档新鲜度、产品复核锚点、生产候选内测材料和提审工作台显式确认护栏。
