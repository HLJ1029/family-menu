...
Humi 1.1 已回到“先完成产品功能，再谈审核”的正确顺序。本轮在最新产品分支修复完整菜品库、三餐手选、已安排菜置顶、征集选人、成员写入边界、隐形食材线索、征集跨会话恢复和家庭协作动态，并把小程序分享改为可验证的原生子页跳转：原生能力缺失或跳转失败时不再误报“已打开”。`docs/humi-1.1-requirement-ledger.md` 现在逐项列出源码和运行证据；家庭订阅是否在 1.1 接入真实支付仍待用户确认，三类微信原生发送框也需在当前候选重新取证。
