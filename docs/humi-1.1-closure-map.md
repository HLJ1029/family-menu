...
Humi 1.1 的核心菜单与协作功能已在当前重构 UI 下完成工程回归，本轮小程序候选 `1.1.73` 已合入主线、部署生产并上传体验版。五类分享卡片均进入微信原生发送页，菜单与清单海报也已接入微信原生图片分享和相册保存；当前真机 P0 明确阻断在微信公众平台尚未给 `https://api.humi-home.com` 配置 downloadFile 合法域名。微信审核和正式发布尚未触发。
...
| 部署 H5/API 并上传新小程序候选 | 10 | 10 | API 已备份并部署 `main@129da03`，生产 smoke 通过；小程序 `1.1.73` 已上传和生成预览二维码 |
...
| **完整上线总计** | **100** | **75** | **工程、生产部署和 1.1.73 体验版已完成；剩余微信域名配置、手机 P0、真实家庭灰度及用户确认后的审核发布** |

剩余 25 分的顺序固定为：用户手机 P0 5 分 -> 真实家庭灰度 10 分 -> 用户确认后审核、发布与监控 10 分。任何阶段发现 P0/P1 都回到修复，不跳级计分。

当前存在 1 个已定位 P0：downloadFile 合法域名缺失。配置并复测后才能继续双海报真机验收；真实灰度约需 2–5 个自然日，微信平台审核通常还需额外等待。审核和正式发布仍需用户分别确认。
...
- 小程序候选：`1.1.73` / `修复身份完善入口` / AppID `wx4040b89f3b363416`。
...
- 小程序体验版：`1.1.73` 已通过微信开发者工具 CLI 上传并生成预览二维码，upload package 54,838 bytes；私有证据 `private:///Users/honglijie/.humi-release-evidence/HUMI-2026-001/20260720T023826Z/miniprogram-upload-1.1.73-20260720T051150Z`。
...
配置生效后重新运行关闭域名跳过选项的 DevTools downloadFile 验证，再用真实微信扫描 `1.1.73` 预览二维码，完成 1.1 核心菜单、五类分享联系人选择与两类海报原生分享/保存验收。
