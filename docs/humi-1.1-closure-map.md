...
Humi 1.1 的核心菜单与协作功能已在当前重构 UI 下完成工程回归，本轮小程序候选为 `1.1.72`。五类分享卡片均进入微信原生发送页，菜单与清单海报也已改为微信原生图片分享和相册保存；完成生产部署与体验版上传后进入用户手机 P0 验收。微信审核和正式发布尚未触发。
...
| 候选交付到远端并合入发布基线 | 10 | 0 | 1.1.72 原生海报链路待合并，合并前不沿用旧候选得分 |
| 部署 H5/API 并上传新小程序候选 | 10 | 0 | 1.1.72 待部署短期海报 API、H5，并上传新的小程序体验版 |
| 用户手机 P0 打开与核心路径 | 5 | 0 | `1.1.72` 需用户手机扫码确认普通启动、五类卡片分享、协作回传和双海报原生动作，不用浏览器结果代替 |
| 真实家庭灰度达到最低样本线 | 10 | 0 | 当前 0/10 真实体验、0/8 完成今晚、0/8 完成清单、0/3 尝试协作 |
| 微信审核、发布与发布后 24 小时监控 | 10 | 0 | 审核、发布、发布后真机和 24 小时监控均未执行，等待候选验收后用户确认 |
| **完整上线总计** | **100** | **55** | **本地功能与工程回归完成；剩余合并部署、1.1.72 体验版、手机 P0、真实家庭灰度及用户确认后的审核发布** |
...
先完成产品功能，再谈审核。
用真实微信扫描 `1.1.72` 预览二维码，完成 1.1 核心菜单、五类分享联系人选择与两类海报原生分享/保存验收。
