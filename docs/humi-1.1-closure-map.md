...
Humi 1.1 的核心菜单与协作功能已完成工程回归。N5a API/H5 已部署；最近已上传体验版 `1.1.74` 精确绑定 `4eb3fbeb`，当前小程序候选 `1.1.75` 尚未归档或上传。微信审核、正式发布、开关和白名单均未触发，下一检查点是为 `1.1.75` 完成不可变封包并取得上传授权，上传后才进入 56 项真机证据收集。
...
| 部署 H5/API 并准备新小程序候选 | 10 | 8 | N5a API/H5 已部署，N5b 小程序 `1.1.74` 已上传；当前完整候选 `1.1.75` 仍待封包和上传授权 |
...
| **完整上线总计** | **100** | **73** | **工程、N5a 生产部署和 1.1.74 历史体验版上传已完成；1.1.75 尚待封包上传，之后才是真机证据、web-view/隐私平台证据、真实家庭灰度及用户确认后的审核发布** |
...
- 小程序候选：`1.1.75` / `Humi 原生骨架完整候选（待上传）` / AppID `wx4040b89f3b363416`。
- 最近已上传体验版：`1.1.74` / `Humi 原生骨架体验版 N5b（4eb3fbeb）` / 精确运行时 `4eb3fbeb6aba886930b3fda652be96e9246eac9e`。
...
- 小程序体验版：`1.1.74` 已从 `4eb3fbeb6aba886930b3fda652be96e9246eac9e` 通过微信开发者工具 CLI 上传并生成预览二维码，未提审、未发布。
...
先为 `1.1.75` 生成新的不可变归档、绑定候选提交并取得明确上传授权；上传成功后再用真实微信完成 56 项登录、推荐、做饭、家庭、五类分享、双海报、提醒、性能和回滚验收。`1.1.74` 的旧二维码不能作为 `1.1.75` 的验收入口。
