...
Humi 1.1 的核心菜单与协作功能已在当前重构 UI 下完成工程回归，H5 已部署，小程序体验版为 `1.1.71`。五类 H5 分享入口均已验证只进入一次原生发送页，开发者工具原生发送框与落地页证据也已通过；下一步是用户手机 P0 验收。微信审核和正式发布尚未触发。
...
| 候选交付到远端并合入发布基线 | 10 | 10 | PR #31 已合入 `main@7733865`，密钥扫描通过 |
| 部署 H5/API 并上传新小程序候选 | 10 | 10 | Pages run `29641009392` 成功；生产 smoke/monitor/readiness 通过；小程序 `1.1.71` 上传与 preview 成功，API 合同未变 |
| 用户手机 P0 打开与核心路径 | 5 | 0 | `1.1.71` 需用户手机扫码确认普通启动、五类分享确实弹出联系人选择、协作回传和海报入口，不用开发者工具结果代替 |
| 真实家庭灰度达到最低样本线 | 10 | 0 | 当前 0/10 真实体验、0/8 完成今晚、0/8 完成清单、0/3 尝试协作 |
| 微信审核、发布与发布后 24 小时监控 | 10 | 0 | 审核、发布、发布后真机和 24 小时监控均未执行，等待候选验收后用户确认 |
| **完整上线总计** | **100** | **75** | **工程候选、H5 部署和 `1.1.71` 体验版已完成；剩余手机 P0、真实家庭灰度及用户确认后的审核发布** |
...
- 小程序候选：`1.1.71` / `明确微信发送步骤并消除假成功` / AppID `wx4040b89f3b363416`。
- H5 部署：PR #31 合并基线 `7733865` 已由 GitHub Pages run `29641009392` 成功部署；五类 H5 分享入口各触发一次原生跳转、无降级重复跳转，两张海报可生成；生产产品与协作 smoke、monitor 和 online readiness 通过，页面错误 0。
...
先完成产品功能，再谈审核。
用真实微信扫描 `1.1.71` 预览二维码，完成 1.1 核心菜单、五类分享联系人选择与海报入口真机验收。
