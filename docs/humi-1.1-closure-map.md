...
Humi 1.1 已形成可供真机验收的生产候选：核心家庭菜单与协作改动已合入 `main`，H5/API 已部署，小程序 `1.1.64` 已上传，线上产品与游客协作 smoke 均为零页面错误。微信审核和正式发布尚未触发；下一步只验证真实微信中的打开、分享和家庭协作，不再扩大 1.1 功能范围。
...
| 候选交付到远端并合入发布基线 | 10 | 10 | PR #3 至 #14 已合并；当前基线用 `git rev-parse origin/main` 核验，不在文档里写死易过期哈希 |
| 部署 H5/API 并上传新小程序候选 | 10 | 10 | 已记录 H5 Pages run `29343259367` 成功；API 已部署并重启；小程序 `1.1.64` 已从 PR #14 主线基线上传；线上迁移、产品与协作 smoke 零错误 |
| 用户手机 P0 打开与核心路径 | 5 | 0 | `1.1.61` 真机白屏已修复；`1.1.64` 需用户手机重新扫码确认，不用开发者工具结果代替 |
| 真实家庭灰度达到最低样本线 | 10 | 0 | 当前 0/10 真实体验、0/8 完成今晚、0/8 完成清单、0/3 尝试协作 |
| 微信审核、发布与发布后 24 小时监控 | 10 | 0 | 审核、发布、发布后真机和 24 小时监控均未执行，等待候选验收后用户确认 |
| **完整上线总计** | **100** | **75** | **工程与部署候选已完成；完整上线完成度 75%；小程序 `1.1.64` 已上传但仍待用户手机 P0、真实家庭灰度和审核发布** |
...
- 小程序上传：`1.1.64` / `清理旧自动餐次记录` / AppID `wx4040b89f3b363416` / package `21.6 KB`。
- H5 部署：PR #14 合并基线 `864e8c8` 已由 GitHub Pages run `29343259367` 成功部署。
...
先完成产品功能，再谈审核。
先用微信开发者工具或体验版打开 `1.1.64`，完成 1.1 核心菜单与协作真机验收。
