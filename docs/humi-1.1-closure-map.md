...
Humi 1.1 的核心菜单与协作功能正在对当前重构 UI 做完整回归。线上小程序仍是 `1.1.69`，但实机分享问题已经重新打开为 P0；修复、五类取证与新体验版完成前，不能把 `1.1.69` 当作验收通过的生产候选。微信审核和正式发布尚未触发。
...
| 候选交付到远端并合入发布基线 | 10 | 0 | 当前分享修复分支尚未合入 `main`，历史合并不能代替本轮候选 |
| 部署 H5/API 并上传新小程序候选 | 10 | 0 | 线上仍是 `1.1.69`，尚未包含本轮分享桥接、五类证据门禁和文案修正 |
...
| **完整上线总计** | **100** | **55** | **本地功能与五类开发者工具验收已完成；当前修复尚待合并、部署和上传新体验版，之后再做手机 P0** |
