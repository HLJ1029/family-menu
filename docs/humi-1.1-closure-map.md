...
`1.1.78` 已完成不可变归档和体验版上传；新的 N5c 权威空会话已绑定 `1.1.78@7606aad` 并保持 `0/56`。下一步需另行授权真实设备执行，再用真实微信完成 56 项登录、推荐、做饭、家庭、五类分享、双海报、提醒、性能和回滚验收。`1.1.75` 的旧空会话和 `1.1.74` 的旧二维码都不能作为 `1.1.78` 的验收入口，本次 N5b-1.1.78 也未执行 preview。
