...
Humi 1.1 的核心菜单与协作功能已完成工程回归。N5a API/H5 已部署；当前体验版 `1.1.75` 已从不可变归档上传，运行时精确绑定 `fbb4938200ef0137c468bd37f3868b94b64b738b`。微信审核、正式发布、开关和白名单均未触发，下一检查点是 N5c 的 56 项 iOS/Android 真机证据收集。
...
| 部署 H5/API 并准备新小程序候选 | 10 | 10 | N5a API/H5 已部署；N5b-1.1.75 已从 `fbb4938` 不可变归档上传为体验版，签名机读证据已通过门禁 |
...
| **完整上线总计** | **100** | **75** | **工程、N5a 生产部署和 1.1.75 体验版上传已完成；剩余是真机证据、web-view/隐私平台证据、真实家庭灰度及用户确认后的审核发布** |
...
- 当前已上传体验版：`1.1.75` / `Humi 原生骨架完整候选（fbb4938）` / AppID `wx4040b89f3b363416` / 精确运行时 `fbb4938200ef0137c468bd37f3868b94b64b738b`。
...
- 小程序体验版：`1.1.75` 已从 `fbb4938200ef0137c468bd37f3868b94b64b738b` 的不可变归档通过微信开发者工具 CLI 上传，包体 `570449 bytes`；本次未生成预览二维码，未提审、未发布。
...
`1.1.75` 已完成不可变归档和体验版上传；下一步需另行授权 N5c，再用真实微信完成 56 项登录、推荐、做饭、家庭、五类分享、双海报、提醒、性能和回滚验收。`1.1.74` 的旧二维码不能作为 `1.1.75` 的验收入口，本次 N5b-1.1.75 也未执行 preview。
