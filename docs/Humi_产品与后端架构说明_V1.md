...
> 编写日期：2026-06-29
> 文档目的：作为"彻底自建后端"决策与上线收尾的统一基线。代码现状部分基于对 `main` 分支真实代码的逐文件核对；目标架构与迁移计划为方案建议，需经 Codex 落地与验收。
...
- **S3 AI 推荐迁出 Supabase（后端）✅ 端到端通过**
  ...
  - `api/server.js` 新增 `POST /recommend`、`POST /explain` 路由，**方案 A**：游客也放行，按客户端 IP 限流（默认 30/分钟，`HUMI_AI_RATE_LIMIT` 可调）保护额度。
  - 验证：11 个单测 + smoke（503/429/404）+ **真实 DeepSeek 端到端**（`curl /recommend` 返回 `source:deepseek` 且结合库存）。
- **S3 前端开关 ✅ 编译通过**
  - 新增 `src/lib/aiViaHumiApi.js`；`aiRecommendation.js`/`aiExplanation.js` 由 `VITE_HUMI_AI_VIA_API==="1"` 切到自建端点。
  - **默认关 = 继续走 Supabase**，部署前不影响生产。`npm run build` 通过。
...
- **微信登录桥**：H5 通过 `wx.miniProgram.postMessage` 请求小程序壳执行 `wx.login`，壳换取会话后用 URL 参数 `humiSession` 回传 H5（`consumeHumiSessionFromUrl`）。
...
- **端点**：`/health`、`/auth/wechat/login`、`/auth/wechat/phone`、`/auth/session/refresh`、`/auth/logout`、`/me`、`/state`(GET/PUT)、`/profile`。
- **认证**：自实现 HMAC-SHA256 签名令牌（类 JWT），30 天 TTL，撤销列表存于 JSON 文件。无独立 refresh token（`refreshToken === accessToken`）。
- **存储**：`HumiStore` 读写**单个 JSON 文件**（`.humi-api-data.json`），结构：`{ users, identities, profiles, states, revokedTokens }`。
- **数据模型**：`states[userId] = 整个 App 状态一坨 JSON`（非关系型，无家庭表）。

### 3.2 Supabase（仍在承载的 3 件事）
| 用途 | 代码位置 | 状态 |
| --- | --- | --- |
| 邮箱登录 | `src/lib/supabase/family.js` + `CloudAccount.jsx` | 小程序渠道已隐藏为开发用；Web 仍可见 |
| **AI 推荐** | `src/lib/supabase/aiRecommendation.js` → Edge Function `recommend-meals` → DeepSeek | 🔴 仍是核心依赖 |
| 推荐解释 | `aiExplanation.js` → Edge Function `explain-recommendation` | 仍依赖 |
...
| 模型 | 每用户整状态 blob | 关系表（families/meal_plans/...） |
| 隔离 | 按 userId | 按 family_id + RLS |
| 历史 | 无 | 仅当前周 |
| 协作 | 无 | family_members（邮箱路径） |
...
### 4.2 微信路径同步（生产主路径）
- 登录后 `loadHumiState` 拉取整状态；之后对 `humiStateSnapshot` 做防抖 `saveHumiState`（`main.jsx` useEffect）。
- **整状态覆盖、最后写入胜出、无多设备合并、无历史版本**。
...
| M4 | 文档与代码不同步（§1.2） | `docs/*` | 误导决策（本次已发生） | Claude 可协助更新文档 |
...
- 中期：若要做历史/协作/多设备（见收费点规划），再引入关系表（meal_plans / shopping_items / families）。
...
2. JSON 文件存储 → Postgres（jsonb 状态表），保留同一 `/state` 接口（修 S2）。**需要数据库连接串。**
