...
当前结论：1.1 台账 70 个需求 ID 均存在，可实现项全部完成并合入 `main`；H5/API 已部署，小程序 `1.1.66` 已上传，线上产品和游客协作 smoke 通过。2026-07-14 用户已确认真实支付、Plus 深度协调、完整版画像、一周计划付费包装及后续菜单体验深化统一列入 1.2；1.1 继续完整提供核心家庭菜单与协作体验。现在不提交微信审核，先完成真实微信验收。
...
- 最新小程序上传：`1.1.66` / `适配新版五入口与家庭协作` / AppID `wx4040b89f3b363416`。
...
2. 当前 H5/API 已部署，小程序 `1.1.66` 已上传，不重复执行部署或上传。
3. 用户在微信开发者工具体验版或真实微信中验收 `1.1.66`。未通过就继续修复并上传新的候选，不进入审核。
