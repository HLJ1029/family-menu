...
当前结论：1.1 台账 70 个需求 ID 均存在，可实现项已按当前重构 UI 完成；本轮小程序候选为 `1.1.71`。2026-07-14 用户已确认真实支付、Plus 深度协调、完整版画像、一周计划付费包装及后续菜单体验深化统一列入 1.2；1.1 继续完整提供核心家庭菜单与协作体验。现在不提交微信审核，先完成真实微信验收。
...
- 最新小程序候选：`1.1.71` / `明确微信发送步骤并消除假成功` / AppID `wx4040b89f3b363416`。
...
2. 本轮 H5 合并部署并上传小程序 `1.1.71` 后，三者以同一主线基线进入真机验收；API 合同未变，不重复部署 API。
3. 用户在微信开发者工具体验版或真实微信中验收 `1.1.71`。未通过就继续修复并上传新的候选，不进入审核。
