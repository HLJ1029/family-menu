...
- 最新产品提交：`1d264a6`（`feat: add crave sheet release template`）；对应 GitHub Pages run `28687866822`，结论 `success`。
...
- `release:wechat:prepare-submit` 只负责创建证据目录、复制审核备注、打开公众平台和证据目录；它不提交审核、不发布、不撤回，也不调用微信开放接口。
