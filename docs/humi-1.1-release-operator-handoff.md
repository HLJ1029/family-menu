...
- 最新小程序上传：`1.1.64`，描述 `清理旧自动餐次记录`，AppID `wx4040b89f3b363416`。
- 已核验 H5：`https://www.humi-home.com/`，Pages run `29343259367` 已成功部署。
...
- 不要在真机验收期间无记录覆盖 `1.1.64`；发现 P0/P1 时先登记、修复、复测，再上传新的 1.1.x 候选。
...
```text
/Users/honglijie/.humi-release-evidence/miniprogram-upload-1.1.64
```
...
- 微信开发者工具 CLI 已完成 `1.1.64` 上传，但本机 CLI 没有提交审核/发布命令。
