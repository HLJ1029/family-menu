...
- `1.1.75@fbb4938` 的私有会话保留为历史 `0/56` 空会话，不得继续为已发生变化的运行时补证据。本地 `1.1.76` 已被后续运行时变更取代，不得上传或创建验收会话；只有为当前 `1.1.77` 生成新的不可变归档、取得上传授权并完成真实上传后，才能创建新会话。候选工作台不会自动发现或展示历史开发者工具二维码。

## N5c 私有会话

只有已上传候选才能从已签名的上传证明创建独立会话。下面的旧命令仅用于验证 `1.1.75@fbb4938` 历史空会话仍可追溯；不得把它作为 `1.1.77` 的验收入口。新的 `1.1.77` 命令必须由其上传证明和精确运行时提交生成，准备器会交叉验证 AppID、版本、运行时、不可变归档 SHA-256 和原始上传回执 SHA-256，调用方不能覆盖这些值。
