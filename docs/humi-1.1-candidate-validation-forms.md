...
每轮发送前运行 npm run release:candidate:dispatch -- --date YYYY-MM-DD，生成只包含今天 U 编号的 candidate-dispatch-YYYY-MM-DD.md/json；再运行 npm run release:candidate:dispatch:workbench -- --date YYYY-MM-DD，生成 candidate-dispatch-workbench-YYYY-MM-DD.html，把逐个发送文案、每个 U 的已发送登记命令、回填模板和日结命令放到同一个私有页面里。
