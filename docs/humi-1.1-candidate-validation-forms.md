...
每轮发送前运行 npm run release:candidate:dispatch -- --date YYYY-MM-DD，生成只包含今天 U 编号的 candidate-dispatch-YYYY-MM-DD.md/json；再运行 npm run release:candidate:dispatch:workbench -- --date YYYY-MM-DD，生成 candidate-dispatch-workbench-YYYY-MM-DD.html，把逐个发送文案、回填模板和日结命令放到同一个私有页面里。
发送当天邀请后运行 npm run release:candidate:invite -- --from-dispatch YYYY-MM-DD --sent-confirmed，把匿名 U 编号标为已邀请；不要记录真实联系人；未带确认参数时不会写入。
