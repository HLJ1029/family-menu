...
是否能不解释就理解【今晚】主按钮。
是否能发现新菜并补进今晚。
完整菜品库是否展示全部菜，且已安排菜是否固定在菜品流上方。
早餐/午餐是否让用户自己选菜，没有擅自记入默认菜。
是否能看懂清单里谁在买、谁买到了。
...
每轮发送前运行 npm run release:candidate:dispatch -- --date YYYY-MM-DD，生成只包含今天 U 编号的 candidate-dispatch-YYYY-MM-DD.md/json；再运行 npm run release:candidate:dispatch:workbench -- --date YYYY-MM-DD，生成 candidate-dispatch-workbench-YYYY-MM-DD.html，把逐个发送文案、微信体验版 `1.1.75` 入口、每个 U 的已发送登记命令、回填草稿命令、回填模板和日结命令放到同一个私有页面里。历史开发者工具二维码不会被自动发现或显示。
