...
当前状态（2026-07-27）：小程序候选 `1.1.75`（`Humi 原生骨架完整候选（待上传）`）尚未归档或上传；最近已上传体验版 `1.1.74` 精确绑定 `4eb3fbeb`。N5a API/H5 已部署，request/downloadFile 探测通过。当前先完成 `1.1.75` 不可变封包和上传授权，上传后再收集 56 项真机、web-view 域名和平台隐私声明证据；`release.candidateValidationReady=false` 时不得进入微信公众平台提交审核。历史版本记录继续保留在本文后半部分。
...
| 保存与分享海报 | H5 生成并上传短期图片，小程序原生页负责发给家人或保存相册 | downloadFile 域名、用户相册写入授权 |
...
- 当前海报由 H5 Canvas 生成，并压缩到 900KB 内上传为 24 小时短期图片。
- 小程序原生页用 `wx.showShareImageMenu` 调起图片分享，用 `wx.saveImageToPhotosAlbum` 在用户主动点击后写入系统相册。
- 必须把 `https://api.humi-home.com` 配置为 downloadFile 合法域名；未授权相册写入时应引导用户打开设置，不得误报成功。
- 降级策略：原生页保留图片长按菜单和顶部小程序卡片分享；临时图片失效后提示回到 Humi 重新生成。
...
- 需要在真机网络下验证 request/downloadFile 合法域名、session 恢复、小程序卡片落地页和海报相册写入授权。
...
- [x] 小程序卡片分享路径覆盖 `crave`、`invite`、`grocery`、`wish` 和 `menu`。
