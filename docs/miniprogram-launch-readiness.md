...
当前状态（2026-07-18）：小程序候选 `1.1.72`（`海报原生分享与保存到相册`）基于当前重构 UI 与完整功能门禁准备；生产 H5/API 和小程序上传后继续做真实微信核心菜单、五类卡片与两类海报验收。当前不直接提交审核；`release.candidateValidationReady=false` 时不得进入微信公众平台提交审核。历史版本同步记录继续保留在本文后半部分。
