...
状态边界：当前 `1.1.78@7606aad` 已从不可变归档上传为体验版，但本包现在只用于预审内容复核，不能作为“可提审”证明。`1.1.75@fbb4938` 是历史空会话版本，`1.1.74@4eb3fbeb` 是上一历史体验版；不得在后台误选旧版本提交。
...
| 微信昵称和头像 | 用户主动完善身份时用于家庭辨认；本地或微信头像可能压缩后上传 Humi API |
| 手机号 | 用户主动绑定时用于账号绑定、登录验证、账号找回、家庭协作安全 |
| 系统相册写入 | 仅在用户主动保存刚生成的海报时写入，不读取相册已有内容 |
| 微信一次性订阅消息 | 仅在用户确认下次做饭时间后请求；拒绝或取消不创建提醒、不重复索取 |
...
平台声明必须与 `docs/wechat-privacy-declaration.json` 一致，并在提交前运行 `npm run release:wechat:privacy:check`。当前平台声明与截图仍待完成，不得把仓库材料误记为微信后台已确认。
