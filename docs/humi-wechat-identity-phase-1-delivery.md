...
| 独立审查修复 | 分支最新提交 | 即时原生登录/退出、所有鉴权 401 统一降级与强制重新登录、失败注销兜底、家庭成员身份投影、移动端账号设置与 JPEG/PNG 结构校验 |
...
- `validate:miniprogram-entry`：`Mini-program entrypoint resilience checks passed.`，动态证明只有 `/pages/identity/index?action=login` 会调用 `wx.login`
- `validate:h5-entry`：`"ok": true`，覆盖即时原生跳转、票据交换、URL 清理、旧/过期 session、首次读取与后续写入 401 统一降级，以及远端注销失败时仍完成本地退出
- `release:product:smoke`：`"ok": true`，对本地候选版本 `http://127.0.0.1:4173/` 检查全部通过；证据目录 `/Users/honglijie/.humi-release-evidence/product-entrypoint-smoke-20260719T125052Z`
- `release:collaboration:smoke`：`"ok": true`，对同一本地候选版本证明游客协作落地页不自动登录
