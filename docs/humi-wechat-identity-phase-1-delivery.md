...
| 独立审查修复 | 分支最新提交 | 即时原生登录/退出、过期会话降级、失败注销兜底、家庭成员身份投影、移动端账号设置与头像结构校验 |
...
- `validate:h5-entry`：`"ok": true`，覆盖即时原生跳转、票据交换、URL 清理、旧/过期/服务端拒绝 session 降级，以及远端注销失败时仍完成本地退出
- `release:product:smoke`：`"ok": true`，对本地候选版本 `http://127.0.0.1:4173/` 检查全部通过；证据目录 `/Users/honglijie/.humi-release-evidence/product-entrypoint-smoke-20260719T123348Z`
