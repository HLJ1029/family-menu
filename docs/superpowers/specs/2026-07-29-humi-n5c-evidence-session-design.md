...
N5c 会话必须绑定当前已上传体验版 `1.1.78`、运行时提交 `7606aadcd03dafe9925885b7fef5c308ecfb73e0`、AppID `wx4040b89f3b363416`、不可变归档和受控签名上传证明。会话初始状态保持真实 `0/56`；工具不得生成伪造的执行时间、媒体、描述文件或通过结果。`1.1.75@fbb4938` 只保留为历史空会话，不得继续补证据。

本设计只增加本地私有验收工具、版本绑定和门禁，不授权生成微信预览、发送消息、配置微信后台、提审、发布、部署、开启原生或 MealRun 开关，或修改家庭白名单。

## 背景与问题

N5b 已将当前 `1.1.78@7606aad` 上传为体验版，但 N5c 仍为 `0/56`。当前本地工具需要持续防止四个执行风险：

- 历史 U001–U020 候选包可能没有绑定当前 `1.1.78` 或运行时提交；
...
- 不改变小程序运行时 `7606aad`，也不重新上传 `1.1.78`；
- 不进入 N5d、N5e 或 N5f。
...
```text
~/.humi-release-evidence/HUMI-2026-001/n5c-1.1.78-<UTC stamp>/
  session.json
  allocation.json
  manifest.json
  run-sheet.md
...
{
  "schemaVersion": 1,
  "sessionId": "n5c-1.1.78-20260802T143902Z-db4ca4ed",
...
1. 验证当前仓库干净且处于隔离分支；
2. 验证候选提交内 `HUMI_PACKAGE_VERSION` 为 `1.1.78`；
3. 验证不可变归档内容等于 `7606aad` 的 `miniprogram/**`；
...
- 新建会话后权威状态仍为 `0/56`，且会话与 `1.1.78@7606aad` 的签名上传证明绑定；
...
- 小程序运行时仍精确等于 `7606aad`，开关为 `0`、白名单为空，未发生微信平台外部动作。
