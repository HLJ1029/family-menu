...
- [ ] **Step 1: 先把 API smoke 改成新合同并确认失败**
...
- [ ] **Step 2: 删除启动静默登录和 session URL 拼接**
...
- [ ] **Step 6: 扩充静态门禁，禁止长期 session 进入 URL**
...
- [ ] **Step 1: 增加失败的 H5 Playwright 票据测试**
...
- [ ] **Step 3: 用 ticket URL reader 替换 session URL reader**
...
- [ ] **Step 5: 删除由登录会话伪造的家庭，并正确接收 `family: null`**
...
- [ ] **Step 6: 让 H5 头像使用服务端身份，而不是相同昵称种子**
...
- [ ] **Step 7: 拦住旧版残留的不完整 H5 session，并同步退出原生缓存**
...
- [ ] **Step 1: 先增加 Supabase Auth 退出门禁并确认失败**
...
- [ ] **Step 2: 删除 `main.jsx` 全部 Supabase 运行路径**
...
- [ ] **Step 6: 运行身份门禁、import 检查、构建和 H5 启动测试**
...
- [ ] **Step 3: 确保 `validate:identity` 覆盖两个脚本**
...
- [ ] **Step 5: 让 API smoke 直接断言数据副作用并单独复跑**
