...
本文档用于把 Humi 1.1.56 从“开发者工具已上传”推进到微信公众平台审核、发布和发布后证据留存。后台页面名称和字段可能随微信平台调整，最终以微信公众平台实时展示为准。
...
- `deploy:api:check` 当前允许因 SSH 失败而不通过；若失败项只有 `ssh-access`，说明 H5/当前生产 API 健康。1.1.56 不新增 API 端点，仍复用已补部署的 1.1.37-1.1.54 服务端增量。
...
- 私有证据目录以 `npm run release:wechat:prepare-submit` 输出为准；当前最新目录是 `/Users/honglijie/.humi-release-evidence/wechat-submit-1.1.56-20260705T011949`。后台截图放这里，仓库只记录结论和私有位置。

## 3. 公众平台操作顺序

1. 登录微信公众平台，进入 Humi 小程序。
2. 检查开发管理/版本管理里是否存在上传版本 `1.1.56`，描述为 `分享卡片转发兜底`。
3. 检查服务器域名：
...
| 证据 | 建议文件名 |
| --- | --- |
| 上传版本 `1.1.56` 列表 | `humi-1.1.56-version-list.png` |
...
如果截图已经放到最新 `wechat-submit-1.1.56-*` 私有目录，优先运行 `release:evidence:record:submit:latest`，它会自动使用最新私有目录登记证据；若目录里只有 README、没有截图或录屏，命令会拒绝登记。`release:evidence:commands -- submit` 会打印手动登记模板；替换时间、提交人、状态和私有证据位置后再运行。
