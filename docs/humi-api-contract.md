...
- 历史兼容基线 1.1.73、上一历史体验版 1.1.74 和当前已上传体验版 `1.1.75@fbb4938` 均使用本合同。三者都包含多家庭定向保存、五类协作分享、协作状态持久化、临时身份隐私收口与短期海报图片交接；API/H5 兼容能力已按 N5a 部署，`1.1.75` 已按 N5b-1.1.75 上传但尚未提审、发布或开启原生开关/白名单。

2026-07-19 身份改造分支说明：本分支已实现主动微信登录、身份资料完善、一次性 H5 票据和无副作用家庭读取；相关 API/H5 兼容能力已部署，`1.1.75@fbb4938` 已上传为体验版。普通启动不得调用微信登录；`POST /auth/wechat/login` 只允许由用户主动点击触发。
