...
- 小程序候选 `1.1.75@fbb4938` 已从不可变归档上传为体验版；`1.1.74@4eb3fbeb` 只作为历史证据，当前仍不提交审核。
...
2. request/downloadFile 探测和 `1.1.75@fbb4938` 体验版上传已通过；取得 N5c 单独授权后执行 56 项真机、web-view 域名和平台隐私声明验收，未通过不进入审核。
3. N5a H5/API 已部署，N5b-1.1.75 已用独立归档和签名机读证据收口；`1.1.74@4eb3fbeb` 的旧上传/预览证据不得复用为当前候选真机证据。
