...
- 小程序候选 `1.1.78@7606aad` 已从不可变归档上传为体验版；`1.1.75@fbb4938` 与 `1.1.74@4eb3fbeb` 只作为历史证据，当前仍不提交审核。
...
- [ ] P0 双海报真机动作：request/downloadFile 探测已通过；仍需菜单海报真实调起图片分享、清单海报真实保存到相册，并覆盖取消与相册权限恢复。
  - 当前证据：正式 AppID、`urlCheck: true` 的真实 `wx.downloadFile` 对 `https://api.humi-home.com/health` 已返回 200；当前只缺菜单海报真实分享、清单海报真实保存及拒绝/取消/权限恢复真机证据，不能用域名探测代替。
...
2. request/downloadFile 探测和 `1.1.78@7606aad` 体验版上传已通过；取得 N5c 单独授权后执行 56 项真机、web-view 域名和平台隐私声明验收，未通过不进入审核。
3. N5a API 与 PR #39 H5 热修复已部署，N5b-1.1.78 已用独立归档和签名机读证据收口；`1.1.75@fbb4938` 与 `1.1.74@4eb3fbeb` 的旧上传/预览证据不得复用为当前候选真机证据。
