...
2. 用户在微信后台配置 downloadFile 合法域名，Codex 关闭调试跳过复测通过后，再执行 `1.1.73` 真机体验验收；未通过就继续修复并上传新的候选，不进入审核。
3. H5/API 与小程序 `1.1.73` 已部署/上传；除非修复产生新代码，否则不重复部署或覆盖候选。
