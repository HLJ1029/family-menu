...
- 小程序候选更新为 `1.1.75`，尚未归档或上传；最近已上传体验版 `1.1.74@4eb3fbeb` 只作为历史证据，暂不提交审核。
...
2. request/downloadFile 探测已通过；当前先完成 `1.1.75` 封包和上传授权，上传后才执行它的 56 项真机、web-view 域名和平台隐私声明验收，未通过不进入审核。
3. N5a H5/API 已部署，N5b 小程序 `1.1.74@4eb3fbeb` 已上传；当前 `1.1.75` 是独立新候选，不能覆盖或复用旧上传证据。
